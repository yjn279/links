 ...
 *   - URL parse failure → input path returned unchanged
 */
...
describe('redirectSystemPath', () => {
  it('returns /shareintent for a share-intent URL with hostname match', () => {
    // iOS scheme: links://dataUrl=linksShareKey/...
    const path = 'links://dataUrl=linksShareKey/some/path';
    expect(redirectSystemPath({ path, initial: true })).toBe('/shareintent');
  });

  it('returns /shareintent for a share-intent URL with key in pathname', () => {
    // Variant where the key appears directly in the pathname segment
    const path = 'links:///dataUrl=linksShareKey/some/path';
  ...
  it('returns the input path for a relative path (URL parse failure)', () => {
